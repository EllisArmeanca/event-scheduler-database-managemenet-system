...
--Formula?i în limbaj natural o problem? pe care s? o rezolva?i folosind un subprogram stocat independent de tip procedur? care s? aib? minim 2 parametri ?i
--s? utilizeze într-o singur? comand? SQL 5 dintre tabelele create. Defini?i minim 2 excep?ii proprii, 
--altele decât cele predefinite la nivel de sistem. Apela?i subprogramul astfel încât s? eviden?ia?i toate cazurile definite ?i tratate.
--primind un id de vorbitor si un id de eveniment afisati numarul de discursuri prestate, participantii evenimentului si locaul unde a fost cazat vorbitorul la evenimentul respectiv
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('A aparut o eroare nea?teptat?: ' || SQLERRM);
END EX9;
/
